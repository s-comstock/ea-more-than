...
function initMediaSetup() {
  ...
  mediaElements.forEach(mediaEl => {
    ...
    const activeMode = !isHoverDevice ? (touchMode || (mode === "hover" ? "autoplay" : mode)) :
      mode;
    const shouldResetOnPause = resetAttr === "true" ? true : resetAttr === "false" ? false :
      activeMode === "hover";
    ...
    const shouldUseClickToggle = activeMode === "click" || (activeMode === "autoplay" &&
    ...
    on(video, "canplay", () => {
      if (shouldBePlaying && isInView && !document.hidden)
        playAttempt();
    });
    on(video, "loadeddata", () => {
      if (shouldBePlaying && isInView && !document.hidden)
